...
object Constants {
  ...
  val SparkConfigurationWarning =
    s"""Please note that by default the tool try to find an "acceptable" number of Spark executors trying not to
       ...
       |If you want to tune your application to achieve a specific runtime, specify ${htmlBold(ParamDuration.option)} while
       |launching the application.""".stripMargin
}
